...
(() => {
 ...
 function commit(label){dirty(label);refreshNumbers();for(const r of project()?.rows||[]){if(r.settlements?.length){const el=grid()?.querySelector('[data-rowid="'+r.id+'"][data-cell="actualNet"]');if(el)el.value=cur(r.settlements.reduce((n,s)=>n+num(s.actualNet)*num(s.qty),0));}}document.querySelectorAll('[data-child-actual]').forEach(el=>el.textContent=cur(calcActualCost(findRow(el.dataset.childActual))));document.querySelectorAll('[data-child-internal]').forEach(el=>el.textContent=cur(calcBBFValue(findRow(el.dataset.childInternal))))}
