...
(() => {
 ...
 function activeBook(p=project(),r){const date=r?.date||p?.dates?.shoot||p?.createdAt?.slice(0,10)||today();return [...books()].filter(b=>b.validFrom<=date).sort((a,b)=>b.validFrom.localeCompare(a.validFrom))[0]}
 ...
 settlementTable=function(p){
  ...
  const html=p.rows.map(r=>{
   ...
   const main='<tr data-row="'+r.id+'" '+(hidden?'hidden':'')+' class="'+(split?'split-parent':'')+'"><td class="lp">'+h.lp+'</td><td class="name">'+field(r,'name',true)+(split?'<button class="sub-toggle" data-split-fold="'+r.id+'">'+icon('layers')+' '+split+'</button>':'')+'</td><td>'+num(r.qty)+'</td><td>'+cur(r.unit)+'</td><td>'+cur(m.sales)+'</td><td>'+cur(r.plannedUnit)+'</td><td>'+cur(m.planned)+'</td><td>'+typeButton(r,r.system||split)+'</td><td>'+field(split?{...r,actualNet:r.settlements.reduce((n,s)=>n+num(s.actualNet)*num(s.qty),0)}:r,'actualNet',r.system||split||['BBF','administracja'].includes(r.actualType))+'</td>'+['actual','internal','overhead','margin','retention'].map(k=>'<td class="num computed '+(k==='actual'?'calculation-start':'')+'" data-metric="'+k+'">'+cur(m[k])+'</td>').join('')+'</tr>';
   return main+(r.settlements||[]).map((s,i)=>'<tr class="settlement-child" data-row="'+s.id+'" '+(hidden||state.ui.splitFold?.[r.id]?'hidden':'')+'><td class="lp">'+h.lp+'.'+(i+1)+'</td><td class="name">'+field(s,'name')+'<div class="child-details">'+'<input type="date" data-child-date="'+s.id+'" aria-label="Data podpozycji" value="'+esc(s.date||'')+'">'+'<select data-child-person="'+s.id+'" aria-label="Osoba w podpozycji"><option value="">Osoba…</option>'+state.crew.map(c=>'<option value="'+c.id+'" '+(c.id===s.personId?'selected':'')+'>'+esc(c.name)+'</option>').join('')+'</select></div></td><td>'+field(s,'qty')+'</td><td colspan="4" class="child-caption">Podpozycja rozliczenia</td><td>'+typeButton(s)+'</td><td>'+field(s,'actualNet',['BBF','administracja'].includes(s.actualType))+'</td><td class="num calculation-start" data-child-actual="'+s.id+'">'+cur(calcActualCost(s,p))+'</td><td class="num" data-child-internal="'+s.id+'">'+cur(calcBBFValue(s,p))+'</td><td colspan="3"></td></tr>').join('');
  }).join('');
 ...
 function splitRow(id){const r=findRow(id);if(!r||r.system)return;const p=parentOf(id)||r;
  if(!p.settlements?.length){p.settlements=[{id:uid(),isSettlementChild:true,name:p.name+' — 1',qty:p.actualType==='BBF'||p.actualType==='Rental BBF'?num(p.qty):1,actualType:p.actualType,actualNet:p.actualNet,bbfPriceId:p.bbfPriceId,bbfItem:p.bbfItem,personId:p.personId,contractId:p.contractId,date:project().dates?.shoot||''}];}
  p.settlements.push({id:uid(),isSettlementChild:true,name:p.name+' — '+(p.settlements.length+1),qty:1,actualType:'',actualNet:0,date:project().dates?.shoot||''});dirty('Dodano podpozycję');redraw();
 }
 ...
 const sidebar=renderSidebar;renderSidebar=function(){sidebar();if(!$('#projectSearch')){const input=document.createElement('input');input.id='projectSearch';input.type='search';input.placeholder='Szukaj projektu lub klienta…';input.setAttribute('aria-label','Wyszukaj projekty');$('#projectGroups').before(input);input.oninput=filterProjects;}filterProjects()};
 ...
 function finishEdit(cancel=false){if(!editing)return;const {el,old}=editing;if(cancel){el.value=old;writeControl(el,old);commit('Anulowano edycję');}else writeControl(el,el.value);el.readOnly=true;el.closest('td')?.classList.remove('grid-editing');editing=null;formatMoneyEditors()}
 ...
 window.bbfSheet={findRow,activeBook,books,redraw,finishEdit,installGrid};
 render();
})();
